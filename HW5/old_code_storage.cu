...
__global__
void coarse_binning_assign(const unsigned int* const d_inputVals,
			   unsigned int* const d_selectors,
			   const size_t numElems,
			   const size_t numBins,
			   const unsigned int binStep)
{
	size_t idx = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__
void coarse_binning_scatter(const unsigned int* const d_inputVals,
							unsigned int* const d_outputVals,
							const size_t numElems,
							const unsigned int * const d_selectors,
							const unsigned int binStep)
...
void coarse_binning(const unsigned int* const d_inputVals,
			   unsigned int* const d_outputVals,
			   const size_t numElems,
			   const size_t numBins,
			   const unsigned int binStep)
{
	...
	unsigned int * d_selectors;
	checkCudaErrors(cudaMalloc(&d_selectors, numBins * numElems * sizeof(unsigned int)));
  
	const dim3 blockSizeAssign (1024, 1, 1);
	const dim3 gridSizeAssign (numElems/blockSizeAssign.x + ((numElems%blockSizeAssign.x) == 0 ? 0 : 1), 1, 1);

 
	coarse_binning_assign<<<gridSizeAssign,blockSizeAssign>>>(d_inputVals, d_selectors, numElems, numBins, binStep);

	unsigned int sum = exclusive_scan<add2, 0>(d_selectors, numBins * numElems);

	coarse_binning_scatter<<<gridSizeAssign,blockSizeAssign>>>(d_inputVals, d_outputVals, numElems, d_selectors, binStep);
...
/*
	...
	histoBlock<<<32,32, numBins*sizeof(unsigned int)>>>(d_vals, d_histo, numElems, numBins);
	cudaDeviceSynchronize(); checkCudaErrors(cudaGetLastError());
}
